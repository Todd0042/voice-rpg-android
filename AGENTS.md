...
## 5. Verification, Building & Wireless Deployment
* Always verify all changes with automated tests: `./gradlew test`.
* Always compile a release/debug APK: `./gradlew assembleDebug`.
* Tag and publish GitHub releases with the compiled APK (`app/build/outputs/apk/debug/app-debug.apk`).
* When a test device is connected via Wi-Fi ADB (e.g. Pixel 11 Pro), verify connectivity and deploy the APK via `adb install -r` upon request.
